...
class SnakeGame extends React.Component{
    ...
    handleKeys = async  (event) => {
        ...
        // if (event.key === 'w' )this.setState((prevState) => ({prev_snake_direction:prevState.snake_direction, snake_direction: "up",move_made:"w" }));
        // else if (event.key === 's')this.setState((prevState) => ({prev_snake_direction:prevState.snake_direction, snake_direction: "down",move_made:"s" }));
        // else if (event.key === 'a')this.setState((prevState) => ({prev_snake_direction:prevState.snake_direction, snake_direction: "left",move_made:"a" }));
        // else if (event.key === 'd')this.setState((prevState) => ({prev_snake_direction:prevState.snake_direction, snake_direction: "right",move_made:"d" }));
    ...
    //   },50)
      ...
      async StartGame(){
        ...
        const response = await axios.post("https://ai-movements.onrender.com/snake/move",{snakeX : snake[0].x,snakeY: snake[0].y,foodX: food.x,foodY:food.y,direction_snake:this.state.snake_direction,direction_food:this.state.food_direction} // Set the content type if needed
          );
            console.log("hello")
        if(response.data){
            console.log("in")
    ...
    async Ai_Make_Move(){
        ...
        document.getElementById("d").classList.remove('highlighted');
                    ...
                    const response = await axios.post("https://ai-movements.onrender.com/snake/move",{snakeX : snake[0].x,snakeY: snake[0].y,foodX: food.x,foodY:food.y,direction_snake:this.state.snake_direction,direction_food:this.state.food_direction});
    ...
    Instance(ctx){
        ...
        if(this.state.start){
            ...
            this.state.food_id = setInterval(()=>{
                ...
                let food = this.state.food
                let unit = this.state.unit
                // this.state.data+=this.CalculateData(snake,food,this.state.prev_snake_direction,this.state.food_direction,this.state.move_made)+"<br>"
                if (
                    (head.x < food.x + unit &&
                    head.x + unit > food.x &&
                    head.y < food.y + unit &&
